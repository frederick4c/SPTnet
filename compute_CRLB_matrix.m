...
CRLB_matrix_HD_frame = zeros(frame_number, 99, D_step, 2, 2);
% (frame, Hurst, diff, CRLB_column1,
   CRLB_column2)warning('off', 'MATLAB:singularMatrix');
tic
for frames = 2:frame_number % from 2 frames to "frame_number"
    ...
    toc % % Visualize the matrix colors = turbo(60);
    % cbh.TickLabels = num2cell(1 : 2 : 30);
for
  i = 2 : frame_number[X, Y] = meshgrid(1 : D_step, 1 : 99);
Z = squeeze(CRLB_matrix_HD_frame(i, :, :, 1, 1));
% surf(X, Y, Z, 'FaceColor', colors(i, :), 'EdgeColor', 'none');
surf(X, Y, Z', 'FaceColor', colors(i*5, :), 'EdgeColor', 'k', 'EdgeAlpha',0.2);
hold on
end
